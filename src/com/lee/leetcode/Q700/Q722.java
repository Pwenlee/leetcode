        ...
        for (int i=0; i<source.length; i++) {
            ...
            if(index != -1){
                ...
                if(index1 != -1 && index1 < index){
                    result.add(s.substring(0, index1));
                    continue;
                }
                StringBuilder temp = index == 0 ? new StringBuilder() : new StringBuilder(s.substring(0, index));
                source[i] = s.substring(index+2);
                for(int j =i; j<source.length; j++,i++){
                    index = source[j].indexOf("*/");
                    if(index != -1){
                        temp.append(source[j].substring(index+2));
                        if(temp.length() > 0) {
                            result.addAll(removeComments(new String[]{temp.toString()}));
                        }
                        break;
                    }
                }
